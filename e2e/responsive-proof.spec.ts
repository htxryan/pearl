...
const test = base.extend<{ seededPage: Page }>({
  seededPage: async ({ page }, use) => {
    ...
    // Wait for content to load - on mobile it's a card list, on desktop it's a table
    await expect(
      page.getByRole("table", { name: "Issue list" }).or(page.getByRole("list", { name: "Issues" })),
...
test.describe("Sidebar responsiveness", () => {
  test("375px: sidebar hidden, hamburger visible", async ({ page }) => {
    ...
    const sidebar = page.locator("aside").filter({ hasNotText: "" }).first();
...
test.describe("Detail view responsive", () => {
  ...
  test("desktop: detail view with fields visible", async ({ page }) => {
    ...
    // Click the title cell of the first row (avoid checkbox column)
    const firstRow = page.getByRole("table", { name: "Issue list" }).locator("tbody tr").first();
    // Click on the row's title text - find the second or third cell
